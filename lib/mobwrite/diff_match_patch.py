...
class diff_match_patch:
  ...
  def patch_apply(self, patches, text, offsets=[]):
    ...
    for patch in patches:
      ...
      if start_loc == -1:
        ...
        if text1 == text2:
          ...
          replacement = self.diff_text2(patch.diffs)
          text = (text[:start_loc] + replacement + text[start_loc + len(text1):])
          
          patch_type = patch.diffs[1][0] # second item in diffs list is the patch itself
          patch_len = len(patch.diffs[1][1])

          if patch_type == self.DIFF_DELETE:
            # print("delete")
            for i in range(len(offsets)):
              if offsets[i] > start_loc + patch_len:
                # offset is beyond the end of the deletion
                offsets[i] = offsets[i] - patch_len

              elif offsets[i] > start_loc:
                # offset is inside deleted area
                offsets[i] = start_loc

          if patch_type == self.DIFF_INSERT:
            # print("insert")
            for i in range(len(offsets)):
              # if offsets[i] - len(nullPadding) > start_loc: # it might be this instead
              if offsets[i] - len(nullPadding) + 2 > start_loc:
                # print("is after", offsets[i], start_loc)
                offsets[i] = offsets[i] + patch_len
            ...
            for (op, data) in patch.diffs:
              ...
              if op == self.DIFF_INSERT:  # Insertion
                text = text[:start_loc + index2] + data + text[start_loc +
                                                               index2:]
                # print("Fuzzy INSERT. If you see this, check cursor position")
                for i in range(len(offsets)):
                  # print("CORRECTIONG", offsets[i])
                  if offsets[i] + len(nullPadding) > start_loc + index2:
                    offsets[i] = offsets[i] + len(data)

              elif op == self.DIFF_DELETE:  # Deletion
                del_start = start_loc + index2
                del_end = start_loc + self.diff_xIndex(diffs, index1 + len(data))

                text = text[:del_start] + text[del_end:]
                # print("Fuzzy DELETE. If you see this, check cursor position")

                for i in range(len(offsets)):
                  if offsets[i] + len(nullPadding) > del_start:
                    if offsets[i] + len(nullPadding) < del_end:
                      offsets[i] = del_start - len(nullPadding)
                    else:
                      offsets[i] = offsets[i] - (del_end - del_start)
              
              if op != self.DIFF_DELETE:
                index1 += len(data)
    # Strip the padding off.
    text = text[len(nullPadding):-len(nullPadding)]
    return (text, results, offsets)
